...
namespace AES 
{
	...
	void encrypt(uint8_t* data, size_t len, const Context& ctx, Mode mode, const uint8_t iv[16])
	{
		if (len % 16 != 0)
		{
			throw std::invalid_argument("data length must be a multiple of 16 bytes");
		}
		...
		case Mode::CBC:
		{
			if (iv == nullptr)
			{
				throw std::invalid_argument("CBC mode requires a non-null IV");
			}
